...
object AkkaSystemModule {
  ...
  class ActorSystemProvider @Inject()(val config: Configuration, val injector: Injector) extends Provider[ActorSystem] {
    override def get() = {
      SigarProvisioner.provision(new File(config[String]("kraken.server.tmpDir")))
      val system = ActorSystem("main-actor-system", config.toConfig)
